...
def parse_filaments(filename):
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            for line in f:
                match = re.search(
                    r"Filament Name: (.*?), Filament Type: (.*?), Filament Vendor: (.*?), .*?Filament ID: (\w+)",
                    line,
                )
                if match:
        ...
        logger.log_error(f"Could not read {filename}: {error}")
    return filaments
...
def find_best_match(bambu_f, spoolman_filaments, used_spool_ids):
    ...
    used_spool_ids = {str(spool_id) for spool_id in used_spool_ids}
    candidates = [
        name
        for name, details in spoolman_filaments.items()
        if str(details["id"]) not in used_spool_ids
    ]
    ...
    exact_matches = [
        ...
        if spoolman_filaments[name]["vendor"] == bambu_f["vendor"]
        and spoolman_filaments[name]["type"] == bambu_f["type"]
    ]
    ...
    if exact_matches:
        matches = difflib.get_close_matches(
            bambu_f["name"],
            [spoolman_filaments[name]["name"] for name in exact_matches],
            n=1,
            cutoff=0.4,
        )
        if matches:
            return next(
                ...
                if spoolman_filaments[name]["name"] == matches[0]
            )
    else:
        # If no exact vendor-type matches, fall back to any vendor/type and match by name
        matches = difflib.get_close_matches(
            bambu_f["name"],
            [spoolman_filaments[name]["name"] for name in candidates],
            n=1,
            cutoff=0.4,
        )
        if matches:
...
def map_filaments():
    ...
    used_spool_ids = {str(value) for value in filament_mapping.values()}
    used_bambu_ids = {key for key in filament_mapping if "::" not in str(key)}
    ...
    pending_filaments = [
        item for item in bambu_filaments.items() if item[1]["id"] not in used_bambu_ids
    ]
    skipped_filaments = []

    while pending_filaments and len(used_spool_ids) < len(spoolman_filaments):
        bambu_name, bambu_data = pending_filaments.pop(0)
        suggested_match = find_best_match(
            bambu_data, spoolman_filaments, used_spool_ids
        )

        if not suggested_match:
            logger.log_error(
                f"No close match for '{bambu_name}', adding to skipped list."
            )
            skipped_filaments.append((bambu_name, bambu_data))
            continue

        logger.log_info(
            f"Suggested match: '{bambu_name}' -> '{suggested_match}' (Spool ID: {spoolman_filaments[suggested_match]['id']})"
        )
        user_input = input(
            "Press Enter to accept, type a Spool ID, or 'pass' to skip: "
        ).strip()
        ...
        if user_input:
            spool_id_lookup = {
                str(spool["id"]): spool["id"] for spool in spoolman_filaments.values()
            }
            selected_spool_id = spool_id_lookup.get(user_input)
        else:
            selected_spool_id = spoolman_filaments.get(suggested_match, {}).get("id")

        if selected_spool_id and str(selected_spool_id) not in used_spool_ids:
            filament_mapping[bambu_data["id"]] = selected_spool_id
            used_spool_ids.add(str(selected_spool_id))
            ...
            if not selected_spool_id:
                ...
                logger.log_error(
                    "\033[91mSpoolman Filament Used in Another Bambulab filament\033[0m"
                )
            skipped_filaments.append((bambu_name, bambu_data))
    ...
    if skipped_filaments:
        ...
        for bambu_name, bambu_data in skipped_filaments:
            ...
            if fallback_match:
                logger.log_info(
                    f"Fallback match for '{bambu_name}' -> '{fallback_match}' (Spool ID: {spoolman_filaments[fallback_match]['id']})"
                )
                user_input = input("Press Enter to accept or type a Spool ID: ").strip()

                spool_id_lookup = {
                    str(spool["id"]): spool["id"]
                    for spool in spoolman_filaments.values()
                }
                selected_spool_id = (
                    spool_id_lookup.get(user_input)
                    if user_input
                    else spoolman_filaments[fallback_match]["id"]
                )
                ...
                if selected_spool_id and str(selected_spool_id) not in used_spool_ids:
                    ...
                    used_bambu_ids.add(bambu_data["id"])
                    logger.log_info(
                        f"Mapped {bambu_name} -> Spool ID: {selected_spool_id}"
                    )

    if len(skipped_filaments) != 0:
        logger.log_error(
            "\033[91mError, skipped filaments are not empty\n Not all filaments are mapped\033[0m"
        )
